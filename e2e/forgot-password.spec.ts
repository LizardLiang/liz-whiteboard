...
test.describe('Forgot password', () => {
  ...
  test('(c) a valid link sets a new password; the old password stops working', async ({
    ...
    await page.goto('/login')
    await page.waitForLoadState('networkidle')
    const oldEmail = page.getByRole('textbox', { name: 'Email' })
    const oldPassword = page.getByRole('textbox', { name: 'Password' })
    await oldEmail.click()
    await oldEmail.pressSequentially(E2E_RESET_USER.email)
    await oldPassword.click()
    await oldPassword.pressSequentially(E2E_RESET_USER.password)
    const oldSignIn = page.getByRole('button', { name: 'Sign in' })
    await expect(oldSignIn).toBeEnabled({ timeout: 10_000 })
    await oldSignIn.click()
