...
describe('logSampledError', () => {
  ...
  it('TC-LOG-BOUNDED: lastLogAt Map evicts oldest entry at 1000-entry capacity', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      logSampledError({ userId: `bulk-user-${i}`, errorClass: 'RBAC_LOOKUP_FAILED', message: 'fill' })
    }
    ...
    logSampledError({ userId: 'trigger-eviction', errorClass: 'EVICT_TEST', message: 'trigger' })
    ...
    logSampledError({ userId: 'bulk-user-0', errorClass: 'RBAC_LOOKUP_FAILED', message: 'after eviction' })
