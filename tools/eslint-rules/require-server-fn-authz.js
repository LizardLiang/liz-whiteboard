 ...
 *
 * HIGH-1 fix: replaced the old single-boolean check with a Set collector
 * so we can require BOTH `findEffectiveRole` AND `hasMinimumRole` when the
 * legacy pattern is used. A discarded `findEffectiveRole(...)` alone does not pass.
 */
