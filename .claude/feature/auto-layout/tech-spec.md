...
- _(removed from the original spec)_ `emitToWhiteboard` from `@/routes/api/collaboration` — **NOT imported** in the revised plan; the server function no longer emits.
   ...
   export function useAutoLayoutOrchestrator(args: {
     ...
     const runRetry = useCallback(async () => {
       ...
       try {
         ...
         setPersistError(err)
         toast.error(
           'Auto Layout could not be saved on retry. Please try again.',
         )
       }
