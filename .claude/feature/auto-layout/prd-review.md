...
- Toast with explicit message text: _"Auto Layout could not be saved — your changes are visible locally but not persisted. Click Retry to save."_
...
- Pre-run warning dialog (P0): when whiteboard contains > 50 tables, a confirmation dialog appears before computation starts: _"This whiteboard has N tables. Auto Layout may take several seconds and cannot be cancelled once started. Existing positions will be overwritten. Continue?"_ with Cancel (default focus) and Run Layout buttons.
- Below 50 tables: no dialog; layout starts immediately.
- Tooltip on the button on > 50 table diagrams: _"Layout cannot be cancelled once started."_
