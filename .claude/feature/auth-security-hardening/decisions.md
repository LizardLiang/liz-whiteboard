...
- **Open:** Engineering picks the form (ESLint rule vs. a meta-test that imports the module and asserts every export is annotated vs. a CI grep). PRD specifies *that* a guard exists, not *how*.
...
- **Rejected:** "`event` is for logging only; client always shows the generic message." Why: misses an opportunity to give users contextual feedback that already does not leak (the `event` is *what action they tried*, not *what resource they targeted*).
